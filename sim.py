...
def run(mapdata_file = None):
    ...
    #
    vol_erupted, t_n = initialize(init_type = p.init_type, file = p.init_file)
    n = 0
    grid_level = 0
    dts = []
    step_times = []
...
def initialize(init_type = None, file = None):
    ...
    if init_type == 'prior_model':
        ...
        #
        I = np.isfinite(g.x)
        phi_S = therm.surface_BL(t_n, g.t_erupted, g.h_n, p.core_temperature)
        ...
        '''
        f = Dataset(mapdata_file, 'r')
        data = f['...']
        t_n = data['valid_time'][:][0]
        x = data['x'][:]
        y = data['y'][:]
        lon = data['lon'][:]
        lat = data['lat'][:]
        h = data['thickness'][:] # assume only have estimated h: Extent x Avg Thickness
        #
        q_init_global = vents.source_term(x,y,t_n)
        g.h_n = h.copy() + p.pos_eps*q_init_global
        # just use B_n == B0 (ignore basal freezing)
        g.B_n = B0.copy()
        # for t_inundation: interpolate between vent area and outside flow extent
        #
        I_flow = g.h_n > 0
        I_vent = q_init_global > 0
        ti = np.zeros(g.x.shape)
        ti[~I_flow] = t_n
        xr = restrict(g.x)
        yr = restrict(g.y)
        hr = restrict(g.h_n)
        I_flowr = hr > 0
        I_ventr = vents.source_term(xr, yr, t_n) > 0
        for i in range(10):
            tir = m.restrict(ti)
            tir  = m.local_mean(tir)
            tir[~I_flowr] = t_n
            tir[I_ventr] = 0.0
            ti = m.interp(tir)
            ti[~I_flow] = t_n
            ti[I_vent] = t_n
            #
        g.t_inundation = ti.copy()
        g.t_erupted = t_n - 0.667 * t_i
        phi_S = therm.surface_BL(t_n, g.t_erupted, g.h_n, p.core_temperature)
        cryst_core = rheo.cryst_avrami(t_n, g.t_erupted, I_flow)
        R_n, Rsurf_n, fluidity_n, abs_grad_S_n, jeffreys_efficiency = rheo.rheo_factor_bl_S_all_outputs(g.h_n, g.B_n, phi_S, cryst_core)
        g.abs_Usurf = Rsurf_n * abs_grad_S_n * g.h_n**2
        #
        vol_erupted = np.sum(g.h_n) * p.dx * p.dy
        '''
        #
        ...
        #-----------------------------------------------------------------------
        # ERROR
        print("----- ERROR: unknown init_source type. Supported types: 'model', None -----")
        #
    #
